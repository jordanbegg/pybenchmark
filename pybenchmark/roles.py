...
class Roles:
    ...
    def get_role(self, role_id: int, headers=None) -> dict:
        ...
        if response.status_code == 200:
            return response.json()
        raise ValueError(f"Unable to retrieve role with id {role_id}. Received {response.json()}")

    @add_auth()
    def create_role(self, name: str, permission_ids: list[int], headers=None) -> dict:
        response = r.post(f"{self.url}/{self.api_route}/", json={"name": name, "permission_ids": permission_ids}, headers=headers)
        if response.status_code == 200:
            return response.json()
    ...
    def delete_role(self, role_id: int, headers=None) -> None:
        ...
        if response.status_code != 200:
            raise ValueError(f"Unable to delete role with id {role_id}. Received {response.json()}")
