...
class MuscleGroups:
    ...
    def get_musclegroup(self, musclegroup_id: int, headers=None) -> dict:
        response = r.get(
            f"{self.url}/{self.api_route}/{musclegroup_id}", headers=headers
        )
        if response.status_code == 200:
            return response.json()
        raise ValueError(
            f"Unable to retrieve musclegroup with id {musclegroup_id}. Received {response.json()}"
        )
    ...
    def delete_musclegroup(self, musclegroup_id: int, headers=None) -> None:
        ...
        if response.status_code != 200:
            raise ValueError(
                f"Unable to delete musclegroup with id {musclegroup_id}. Received {response.json()}"
            )

    @add_auth()
    def update_musclegroups(
        self, musclegroup_id: int, new_name: str, headers=None
    ) -> dict:
        response = r.patch(
            f"{self.url}/{self.api_route}/{musclegroup_id}",
            json={"name": new_name},
            headers=headers,
        )
        if response.status_code == 200:
            return response.json()
        raise ValueError(
            f"Unable to update musclegroup with id {musclegroup_id}. Received {response.json()}"
        )
