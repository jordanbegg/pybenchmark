...
class Weights:
    ...
    @add_auth()
    def create_weight(self, weight: str, user_id: int, date: dt.date, headers=None) -> dict:
        response = r.post(f"{self.url}/{self.api_route}/", json={"weight": weight, "user_id": user_id, "date": date}, headers=headers)
        if response.status_code == 200:
            return response.json()
