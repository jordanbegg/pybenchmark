...
class Permissions:
    ...
    def get_permission(self, permission_id: int, headers=None) -> dict:
        response = r.get(f"{self.url}/{self.api_route}/{permission_id}", headers=headers)
        if response.status_code == 200:
            return response.json()
        raise ValueError(f"Unable to retrieve permission with id {permission_id}. Received {response.json()}")
    ...
    def delete_permission(self, permission_id: int, headers=None) -> None:
        response = r.delete(f"{self.url}/{self.api_route}/{permission_id}", headers=headers)
        if response.status_code != 200:
            raise ValueError(f"Unable to delete permission with id {permission_id}. Received {response.json()}")
